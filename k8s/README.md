...
# apollo-mcp-server.yaml's own ConfigMap — see k8s/qa/README.md's
# identical step for why this is separate from hive-gateway's.
...
# config.yaml + the 4 hand-written operation files — see
# k8s/qa/README.md's identical step.
kubectl create configmap apollo-mcp-server-config \
  ...
  -n mfe-demo --dry-run=client -o yaml | kubectl apply -f -
kubectl apply -f apollo-mcp-server.yaml
